    ...
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_films_afficher_data = """SELECT id_eleve, nom_pers, prenom_pers, date_naiss, mail, pays,
                                                            GROUP_CONCAT(intitule_genre) as GenresFilms FROM t_cours
                                                            RIGHT JOIN t_cours ON t_cours.id_eleve = t_cours.FK_Eleve_avoir_cour
                                                            LEFT JOIN t_cours ON t_cours.id_cours = t_cours.FK_el_avoir_Cours
                                                            GROUP BY id_cours"""
                if id_film_sel == 0:
